                ...
                <div class="card-body">
                    <div class="chat-box" style="max-height: 400px; overflow-y: auto;">
                        @foreach ($messages as $message)
                            <div class="{{ $message->from_role == 'admin' ? 'sent' : 'received' }} mb-2">
                                <strong>{{ $message->from_role == 'admin' ? 'You' : $client->name }}:</strong>
                                <p>{{ $message->message }}</p>
                    ...
                    <form action="{{ route('admin-store-message', $client->id) }}" method="POST">
                        @csrf
